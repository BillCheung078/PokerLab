...
- **Activity:** Bonus Configuration Hardening - Introduced a centralized runtime configuration layer to replace scattered hard-coded operational settings across the application. Added a dedicated configuration package to define defaults and load environment-variable overrides for HTTP server settings, session behavior, shared SSE stream behavior, and simulation runtime tuning. Refactored application startup to load configuration once in the server entrypoint and inject the resolved values into the session manager, simulation engine, table manager, and HTTP layer instead of relying on package-local constants. Configured key runtime parameters such as the per-session max table limit, session cookie name and security flags, stream heartbeat interval, replay limit, subscriber buffer size, simulation pacing, and runtime event history retention. Added configuration tests to verify default values and environment-based overrides, and re-ran the full Go test suite to confirm that the configuration refactor preserved existing lifecycle, streaming, and simulation behavior.
